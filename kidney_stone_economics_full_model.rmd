...
## Tabulate Percentage risks 
```{r}
malignancy_grand_total %>%
  mutate(
    pct_expected = (grand_total_expected_malignancies / total_patients) * 100,
    pct_lower = (grand_total_expected_malignancies_lower / total_patients) * 100,
    pct_upper = (grand_total_expected_malignancies_upper / total_patients) * 100
  ) %>%
  group_by(cohort_type,
          risk_status,
           auc_target) %>%
  ...
  fmt_number(
    columns = c(pct_expected, pct_lower, pct_upper),
    decimals = 4
  ) %>%
  fmt_number(
    columns = total_patients,
...
## Tabulate Total Malignancies per Cohort Type
```{r}
malignancy_grand_total %>%
  group_by(cohort_type, auc_target) %>%
  summarise(
    total_patients = sum(total_patients, na.rm = TRUE),
    grand_total_expected_malignancies = sum(grand_total_expected_malignancies, na.rm = TRUE),
    grand_total_expected_malignancies_lower = sum(grand_total_expected_malignancies_lower, na.rm = TRUE),
    grand_total_expected_malignancies_upper = sum(grand_total_expected_malignancies_upper, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    pct_expected = (grand_total_expected_malignancies / total_patients) * 100,
...
comparisons <- list(
  c("Maximum FU, CT", "Maximum FU, US"),
  c("Minimum FU, CT", "Minimum FU, US"),
  c("Maximum FU, CT", "Minimum FU, CT"),
  c("Maximum FU, US", "Minimum FU, US")
)
...
```{r}
# Define colours
risk_colours <- c("All" = "#4682B4", "High Risk" = "#CD5C5C", "Low Risk" = "#66B266")

baseline_malignancies_perc_line_plot <- malignancy_plot_data %>%
  rbind(all_malignancies) %>%
  mutate(
    auc = as.numeric(str_remove(as.character(auc_target), "AUC ")),
    cohort_type = factor(
      cohort_type,
      levels = c(
        "Minimum FU, XR + US",
  ...
  ggplot(aes(x = auc, y = percentage, colour = risk_status, fill = risk_status)) +
  geom_smooth(method = "loess", se = FALSE, linewidth = 1) +
  geom_ribbon(aes(ymin = percentage_lower, ymax = percentage_upper), alpha = 0.2, colour = NA, show.legend = FALSE) +
  facet_wrap(~ cohort_type) +
  ...
  labs(
    ...
    x = "AUC",
    y = "Additional Malignancies vs Baseline (%)\n\n\u2190 Fewer malignancies   |   More malignancies \u2192",
    colour = "Risk Status",
    fill = "Risk Status"
  ) +
  geom_ribbon(
    aes(x = auc, ymin = 0, ymax = 0.045, group = risk_status),
    fill = "#FFCCCC", alpha = 0.1, colour = NA
  ) +
  # Shading below y = 0 (pale green)
  geom_ribbon(
    aes(x = auc, ymin = -0.01, ymax = 0, group = risk_status),
    fill = "#CCFFCC", alpha = 0.1, colour = NA
  ) +
  scale_y_continuous(labels = function(x) paste0(x, "%")) + scale_colour_manual(values = risk_colours) +
  scale_fill_manual(values = risk_colours) +
  theme(legend.position = "none")
baseline_malignancies_perc_line_plot
```
\newpage
...
## Line plot
```{r}
summary_df_full_cost_line_plot <- summary_df_full_cost_data %>%
  mutate(
    auc = as.numeric(str_remove(auc_label, "AUC ")),
    cohort_type = factor(cohort_type,
                         levels = c(
                           "Minimum FU, XR + US",
                           "Minimum FU, US",
  ...
  ggplot(aes(x = auc, y = total_cost, fill = risk_status, color = risk_status)) +
  ...
  geom_ribbon(
    aes(x = auc, ymin = 0, ymax = 35, group = risk_status),
    fill = "#FFCCCC", alpha = 0.1, colour = NA
  ) +
  # Shading below y = 0 (pale green)
  geom_ribbon(
    aes(x = auc, ymin = -20, ymax = 0, group = risk_status),
    fill = "#CCFFCC", alpha = 0.1, colour = NA
  ) + 
  geom_hline(yintercept = 0, linetype = "dashed") + 
...
summary_df_full_cost_line_plot
```
\newpage
...
summary_df_full_cost_data %>% group_by(cohort_type) %>% ggplot(aes(x = cohort_type, y = total_cost, fill = auc_label)) +
  ...
  labs(
    title = "Annual Cost for Maximum EAU Follow-Up of those with Clinically Significant Disease",
    x = "Cohort Type",
    y = "Minimum Annual Cost of Follow-up (£ Millions)",
    fill = "Risk Status"
  )
```
\newpage
...
baseline_costs_rec_only <- summary_df_rec_only %>%
  filter(auc_label == "AUC 0.55") %>%
  filter(cohort_type == "Minimum FU, US") %>%
  mutate(auc_label == "Baseline")
...
summary_df_appts %>% group_by(auc_label) %>% ggplot(aes(x = auc_label, y = total_appts, fill = risk_status)) +
  ...
  labs(
    title = "Minimum Annual Number of Appointments for EAU Follow-Up of those with Clinically Significant Disease",
    x = "Cohort Type",
...
## Line plot
```{r}
summary_df_appts_line_plot <- summary_df_appts %>%
  mutate(
    auc = as.numeric(str_remove(auc_label, "AUC ")),
    cohort_type = factor(cohort_type,
                         levels = c(
  ...
  ggplot(aes(x = auc, y = total_appts, colour = risk_status, fill = risk_status)) +
  geom_smooth(method = "lm", se = TRUE, linewidth = 1) +
  facet_wrap(~ cohort_type) +
  theme_bw() +
  geom_abline(y = 0, linetype = "dashed") + 
  theme(
    strip.text = element_text(size = 10)
  ) +
  geom_ribbon(
    aes(x = auc, ymin = 0, ymax = 150, group = risk_status),
    fill = "#FFCCCC", alpha = 0.1, colour = NA
  ) +
  # Shading below y = 0 (pale green)
  geom_ribbon(
    aes(x = auc, ymin = -250, ymax = 0, group = risk_status),
    fill = "#CCFFCC", alpha = 0.1, colour = NA
  ) +
  labs(
     title = "Minimum Annual Number of Appointments",
    x = "AUC",
    y = "Appointments Difference (n, 000s)\n\n\u2190 Fewer appointments   |   More appointments \u2192",
    colour = "Risk Status",
    fill = "Risk Status"
  ) + ylim(-250, 150)  + 
  scale_colour_manual(values = risk_colours) +
...
summary_df_full_qol_plot <- summary_df_full_qol_data %>% 
  mutate(
    ...
    # CORRECTED: Conservative CI calculation
...
## Line plot
```{r}
summary_df_full_qol_line_plot <- summary_df_full_qol_data %>% 
  mutate(
    cohort_type = factor(cohort_type, levels = c(
      "Minimum FU, XR + US", "Minimum FU, US", "Minimum FU, CT",
      "Maximum FU, XR + US", "Maximum FU, US", "Maximum FU, CT"
  ...
  labs(title = "Quality of Life Difference from Baseline (Minimum FU, XR + US)",
       ...
       fill = "Risk Status") +
  geom_ribbon(
    aes(x = auc_numeric, ymin = 0, ymax = 0.3, group = risk_status),
    fill = "#FFCCCC", alpha = 0.1, colour = NA
  ) +
  # Shading below y = 0 (pale green)
  geom_ribbon(
    aes(x = auc_numeric, ymin = -0.2, ymax = 0, group = risk_status),
    fill = "#CCFFCC", alpha = 0.1, colour = NA
  )

summary_df_full_qol_line_plot
```
\newpage
...
summary_df_full_cost_plot_paper1 <- summary_df_full_cost_data %>%
  ...
  mutate(
    ...
    # Split into follow-up length and imaging modality
    fu_length = ifelse(grepl("Minimum", cohort_type), "Minimum FU", "Maximum FU"),
    imaging = case_when(
      grepl("XR \\+ US", cohort_type) ~ "XR + US",
      grepl("US", cohort_type) & !grepl("XR", cohort_type) ~ "US",
      grepl("CT", cohort_type) ~ "CT"
    ),
    fu_length = factor(fu_length, levels = c("Minimum FU", "Maximum FU")),
    imaging = factor(imaging, levels = c("XR + US", "US", "CT")),
    total_cost = case_when(
      risk_status == "All" ~ (total_cost - baseline_cost_all),
      risk_status == "High Risk" ~ (total_cost - baseline_cost_hr),
      risk_status == "Low Risk" ~ (total_cost - baseline_cost_lr),
    )) %>%
  ggplot(aes(x = risk_status, y = total_cost, fill = risk_status)) +
  geom_col(width = 0.7, color = "black") +
  facet_grid(fu_length ~ imaging) +
  scale_fill_manual(values = risk_colours) +
  theme_bw() +
  theme(
...
summary_df_appts_plot_paper1 <- summary_df_appts %>% 
  ...
  ggplot(aes(x = risk_status, y = total_appts, fill = risk_status)) +
  geom_col(width = 0.7, color = "black") +
  facet_grid(fu_length ~ imaging) +  
  scale_fill_manual(values = risk_colours) +
  theme_bw() +
  ...
  labs(
    title = "Minimum Annual Appointments",
    x = "Imaging Modality",
    y = "Appointments Difference (n, 000s)",
    fill = "Risk Status"
  ) 
...
baseline_malignancies_perc_plot_paper1 <- malignancy_plot_data %>%
  ...
  ggplot(aes(x = risk_status, y = percentage, fill = risk_status)) +
  geom_col(color = "black", width = 0.7) +
  geom_errorbar(aes(ymin = percentage_lower, ymax = percentage_upper), width = 0.2) +
  facet_grid(fu_length ~ imaging) +
  labs(
    title = "Lifetime Malignancies",
    x = "Imaging Modality",
...
summary_df_full_qol_plot_paper1 <- summary_df_full_qol_data %>% 
  ...
  ggplot(aes(x = risk_status, y = mean_qaly_5yr_comp, fill = risk_status)) +
  geom_col(width = 0.7, color = "black") +
  geom_errorbar(aes(ymin = lower_qaly_5yr_comp, ymax = upper_qaly_5yr_comp), width = 0.2) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray50") +
  facet_grid(fu_length ~ imaging) +
  scale_fill_manual(values = risk_colours) +
  theme_bw() +
...
### Cost-effectiveness Analyses - USIQoL
```{r}
auc_0.55_icer_plots_usiqol <- auc_0.55_icer_plots & theme_bw()
...
## Paper 2
```{r}
summary_df_full_cost_plot <- summary_df_full_cost_plot + 
  scale_fill_manual(values = risk_colours)

summary_df_appts_plot <- summary_df_appts_plot + 
  scale_fill_manual(values = risk_colours)

baseline_malignancies_perc_plot <- baseline_malignancies_perc_plot + 
  scale_fill_manual(values = risk_colours)

summary_df_full_qol_plot <- summary_df_full_qol_plot + 
  scale_fill_manual(values = risk_colours)
...
paper_2_plot <- (
  ...
  plot_annotation(
    tag_levels = "A",
    title = "Variations in Length/Imaging Modality of Follow-up Compared to Current Minimum Standard of Care"
  )
...
# 15. QoL - EQ-5D
```{r}
source("Model/11. Quality of Life - EQ-5D old v2.R", echo = TRUE)
```
\newpage
...
## Line plot
```{r}
summary_df_full_qol_eq_5d_line_plot <- summary_df_full_qol_data_eq_5d %>% 
  mutate(
    cohort_type = factor(cohort_type,
                         levels = c(
                           "Minimum FU, XR + US",
  ...
  labs(title = "Quality of Life",
       x = "AUC",
       y = "5yr QALYs Difference\n\n\u2190 Lower QALYs   |   Higher QALYs \u2192",
       colour = "Risk Status",
       fill = "Risk Status") +
  geom_ribbon(
    aes(x = auc_numeric, ymin = 0, ymax = -0.2, group = risk_status),
    fill = "#FFCCCC", alpha = 0.1, colour = NA
  ) +
  # Shading below y = 0 (pale green)
  geom_ribbon(
    aes(x = auc_numeric, ymin = 0.3, ymax = 0, group = risk_status),
    fill = "#CCFFCC", alpha = 0.1, colour = NA
  )  + 
  scale_colour_manual(values = risk_colours) +
  scale_fill_manual(values = risk_colours)

summary_df_full_qol_eq_5d_line_plot
```
\newpage
...
summary_df_full_qol_plot_eq5d_paper1 <- summary_df_full_qol_data_eq_5d %>% 
  ...
  labs(title = "Quality of Life - EQ-5D",
       x = "Imaging Modality",
       y = "5yr QALYs Difference",
       fill = "Risk Status")
```
