...
malignancy_plot_data %>%
  ...
  labs(
    title = "Difference in Expected Lifetime Malignancies by Cohort Type and Risk Status Compared to Baseline",
    x = "AUC Target",
    y = "Expected Additional Lifetime Malignancies Compared to Baseline (%)",
    fill = "AUC Target",
    caption = "Pairwise chi-squared tests; * p<0.05, ** p<0.01, *** p<0.001, ns = not significant"
  ) +
  scale_y_continuous(labels = function(x) paste0(x, "%"),
...
summary_df_full_cost_data %>%
  mutate(total_cost = case_when(
    risk_status == "All" ~ (baseline_cost_all - total_cost),
    risk_status == "High Risk" ~ (baseline_cost_hr - total_cost),
    risk_status == "Low Risk" ~ (baseline_cost_lr - total_cost),
  )) %>%
  group_by(auc_label) %>%
  ...
  labs(
    title = "Annual Cost Difference for EAU Follow-Up of those with Clinically Significant Disease",
    x = "Cohort Type",
...
summary_df_appts %>%
  mutate(total_appts = case_when(
    risk_status == "All" ~ (baseline_appt_all - total_appts),
    risk_status == "High Risk" ~ (baseline_appt_hr - total_appts),
    risk_status == "Low Risk" ~ (baseline_appt_lr - total_appts),
  )) %>%
  group_by(auc_label) %>%
  ...
  labs(
    title = "Annual Number of Appointments Difference for EAU Follow-Up of those with Clinically Significant Disease",
    x = "Cohort Type",
    y = "Annual Number of Appointments Difference compared to baseline (n)",
    fill = "Risk Status"
  )
```
