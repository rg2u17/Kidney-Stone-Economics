...
# 11. QoL - EQ-5D
```{r}
source("Model/11. Quality of Life - EQ-5D.R", echo = TRUE)
```
\newpage
...
# 12. Calculate ICER with EQ-5D data
```{r}
...
# 13. Find CT Cost Effectiveness Threshold for Current Standard of Care
```{r}
source("Model/13. Find CT CE Threshold.R")
```
\newpage
...
# 14. DCA Analysis
```{r}
source("Model/14. DCA analysis.R")
```
## AUC 0.55
```{r}
dca_0.55$plot
...
# 15. Summary plots with EQ-5D data
## Paper 1
### Collate data
```{r}
# Define colours
...
### Cost-effectiveness Analyses - EQ5D
```{r}
auc_0.55_icer_plots_eq5d <- auc_0.55_icer_plots & theme_bw()
...
## Paper 2
```{r}
summary_df_full_cost_line_plot <- summary_df_full_cost_line_plot + 
  scale_fill_manual(values = risk_colours)

summary_df_appts_line_plot <- summary_df_appts_line_plot + 
  scale_fill_manual(values = risk_colours)

baseline_malignancies_perc_line_plot <- baseline_malignancies_perc_line_plot + 
  scale_fill_manual(values = risk_colours)

summary_df_full_qol_eq_5d_line_plot <- summary_df_full_qol_eq_5d_line_plot + 
  scale_fill_manual(values = risk_colours)
