...
baseline_malignancies_perc_plot <- malignancy_plot_data %>%
  ...
  labs(
    title = "Additional Expected Lifetime Malignancies vs Baseline",
    x = "AUC Target",
...
summary_df_appts_plot <- summary_df_appts %>%
  ...
  labs(
    title = "Annual Number of Appointments Difference",
    x = "Cohort Type",
...
summary_df_full_qol_plot <- summary_df_full_qol_data %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        strip.text = element_text(size = 10)) +
  labs(title = "QALYS compared to Baseline",
       x = "Cohort Type",
       y = "Mean QALYs (USIQOL/5yrs)",
       fill = "Risk Status") + ylim(-0.015,0.015)
...
### Collate data
```{r}
# Designate comparisons
compare_to_minXR <- list(
  c("Minimum FU, XR", "Minimum FU, XR + US"),
  c("Minimum FU, XR", "Minimum FU, CT"),
  c("Minimum FU, XR", "Maximum FU, XR"),
  c("Minimum FU, XR", "Maximum FU, XR + US"),
  c("Minimum FU, XR", "Maximum FU, CT")
)

compare_to_minXR_malig <- list(
  c("Minimum FU, XR", "Maximum FU, XR"),
  c("Minimum FU, XR", "Minimum FU, CT"),
  c("Minimum FU, XR", "Maximum FU, CT")
)
...
baseline_malignancies_perc_plot_paper1 <- malignancy_plot_data %>%
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        strip.text = element_text(size = 10)) + geom_signif(
    comparisons = compare_to_minXR_malig,
    map_signif_level = TRUE,
    step_increase = 0.1,
    textsize = 3
  )
...
summary_df_full_qol_plot_paper1 <- summary_df_full_qol_data %>% 
  ...
  labs(title = "Quality of Life",
       ...
       fill = "Risk Status") + ylim(-0.015,0.015) + geom_signif(
    comparisons = compare_to_minXR,
    map_signif_level = TRUE,
    step_increase = 0.1,
    textsize = 3
